...
struct resultForOutput
{
	...
	double mHSquared;
	double mH_in_GeV;
	int resultFlag; //0:no error; 1:no convergence(frosen or maxcount reached), 2:error during iteration, 3:not only 1 extremum or negative curvature above teh vev; 
...
//true if there is negative curvature for a value larger than min
